<template>
  <div class="detect-container">
    ...
    <div class="page-header">
      <h2>脑肿瘤辅助分割图像与分析</h2>
      <p class="subtitle">使用分割模型输出病灶多边形、二值掩码与半透明可视化叠加图</p>
    </div>
    ...
    <div class="image-compare-area" v-if="currentRecord">
        ...
        <el-col :span="12">
          <div class="image-card">
            ...
            <div class="image-wrapper">
              ...
              />
              <el-empty v-else description="等待分割">
            ...
            <div class="detection-summary" v-if="detectionData && detectionData.tumor_detected">
              <el-descriptions :column="2" size="small" border>
                ...
                <el-descriptions-item label="多边形数量">
                  {{ detectionPolygons.length }} 个
                </el-descriptions-item>
      ...
      <el-row :gutter="20" class="image-row">
        ...
        <el-col :span="12">
          <div class="image-card">
            <div class="image-title">掩码二值图像</div>
            <div class="image-wrapper">
              <img
                v-if="maskImageUrl"
                :src="maskImageUrl"
        ...
        <el-col :span="12">
          <div class="image-card">
            <div class="image-title">可视化叠加图</div>
            <div class="image-wrapper">
              <img
      ...
      <el-card v-if="detectionPolygons.length > 0" class="polygon-card">
        <template #header>
          <div class="card-header">
            <span>分割多边形与置信度</span>
          </div>
        </template>
        <el-table :data="detectionPolygons" size="small" style="width: 100%">
          <el-table-column prop="id" label="序号" width="80" />
          <el-table-column label="类型">
            <template #default="{ row }">{{ mapTumorTypeName(row.tumor_type || row.label) }}</template>
          </el-table-column>
          ...
          <el-table-column label="面积(px²)" width="140">
            <template #default="{ row }">{{ Number(row.area_pixels || 0).toFixed(2) }}</template>
          </el-table-column>
        </el-table>
      </el-card>
    </div>
...
import { fetchAiConfig, fetchGlobalConfig, fetchModels, formatModelOptions, getBackendBaseUrl, getCachedGlobalConfig, getThreshold, isWorkPageAutoRefreshEnabled, onConfigUpdated } from '@/utils/config'
...
const segmentService = ref(getCachedGlobalConfig().defaultMaskSegmentService || 'unet')
const inputSize = ref(512)
const inputSizeOptions = [
  { label: '256x256', value: 256 },
  { label: '512x512 默认值', value: 512 },
...
const toFileUrl = (value) => {
  ...
  return `${getBackendBaseUrl()}/files/${path}`
}

const originalImageUrl = computed(() => toFileUrl(currentRecord.value?.originalImageUrl))

const resultImageUrl = computed(() => toFileUrl(
  currentRecord.value?.resultImageUrl ||
  detectionData.value?.result_image_url
))

const maskImageUrl = computed(() => toFileUrl(
  detectionData.value?.mask_image_url ||
  detectionData.value?.maskImageUrl
))

const overlayImageUrl = computed(() => toFileUrl(
  detectionData.value?.overlay_image_url ||
  detectionData.value?.overlayImageUrl
))
...
const getMaxConfidencePolygon = (polygons = []) => {
  const normalizedPolygons = normalizeDetectionPolygons(polygons)
  if (normalizedPolygons.length === 0) return null
  return normalizedPolygons.reduce((max, polygon) => {
    const currentConfidence = Number(polygon?.confidence ?? 0)
    const maxConfidence = Number(max?.confidence ?? 0)
    return currentConfidence > maxConfidence ? polygon : max
  }, normalizedPolygons[0])
}
...
const getTumorTypeName = (data, record = currentRecord.value) => {
  if (!data) return '未知'
  const maxConfPolygon = getMaxConfidencePolygon(data.polygons)
  ...
  const candidates = [
    getBoxTumorTypeValue(maxConfPolygon),
...
const createReportHeaderCanvas = (record, data) => {
  ...
  drawText(ctx, '脑肿瘤辅助分割与分析报告', width / 2, 16, {
    align: 'center',
    color: '#1f2933',
  ...
  drawText(ctx, `分割模型: ${record?.modelName || selectedModelName.value}`, width - 35, 94, { align: 'right', color: '#687782', font: '12px "Microsoft YaHei", "SimHei", Arial, sans-serif' })
  ...
  const polygonCountText = `${normalizeDetectionPolygons(data.polygons).length} 个`
  ...
  drawText(ctx, '多边形数量:', 58, 206, { color: '#687782' })
  drawText(ctx, polygonCountText, 142, 206, { color: '#1f2933' })
...
const detectionData = computed(() => {
  ...
  try {
    ...
    data.polygons = normalizeDetectionPolygons(data.polygons)
    // 分割页优先从 polygons 中提取最高置信度，兼容旧 boxes 数据
    if (data.polygons && data.polygons.length > 0) {
      const maxConfPolygon = getMaxConfidencePolygon(data.polygons)
      data.confidence = Number(maxConfPolygon?.confidence ?? data.confidence ?? 0)
...
const detectionPolygons = computed(() => detectionData.value?.polygons || [])
...
const detectStatusText = computed(() => {
  const status = currentRecord.value?.detectStatus
  const texts = { 0: '待分割', 1: '分割中', 2: '分割完成', 3: '分割失败' }
  ...
  try {
    const res = await request.post(`/detect/startDetect/${currentRecord.value.id}`, null, {
      params: {
        ...
        mode: SEGMENT_MODE,
        segmentService: segmentService.value,
    ...
    if (res.code === '200') {
      ...
      ElMessage.error(res.msg || '分割失败')
    }
  } catch (error) {
    console.error('分割失败:', error)
    ElMessage.error(error.response?.data?.msg || '分割请求失败')
  } finally {
...
const exportReport = async () => {
  ...
  try {
    ...
    // 下载PDF
    const fileName = `脑肿瘤辅助分割与分析报告_${currentRecord.value.id}_${new Date().toISOString().split('T')[0]}.pdf`
    pdf.save(fileName)
...
const uploadToFeishu = async () => {
  ...
  try {
    ...
    const fileName = `脑肿瘤辅助分割与分析报告_${timestamp}_${userName}.pdf`
...
const loadModels = async () => {
  ...
  try {
    const res = await fetchModels('mask')
    if (res.code === '200') {
...
const applyConfig = async () => {
  confThreshold.value = await getThreshold('mask', 0.45)
  try {
    const res = await fetchGlobalConfig()
    if (res.code === '200' && res.data?.defaultMaskSegmentService) {
      segmentService.value = res.data.defaultMaskSegmentService
    }
  } catch (error) {
    console.warn('加载Mask分割服务配置失败，使用本地缓存', error)
    segmentService.value = getCachedGlobalConfig().defaultMaskSegmentService || 'unet'
  }
}
...
onMounted(() => {
  ...
  removeConfigListener = onConfigUpdated(({ thresholds }) => {
    if (thresholds?.mask !== undefined) {
      confThreshold.value = Number(thresholds.mask)
    }
  })
})
...
.operation-area {
  ...
  --input-size-control-width: 136px;
  --threshold-control-width: 120px;
  --primary-action-width: 112px;
  --feishu-config-width: calc(var(--upload-control-width) + var(--model-control-width) + var(--input-size-control-width) + var(--threshold-control-width) + var(--operation-gap) * 3);
  display: flex;
...
.input-size-select {
  width: var(--input-size-control-width);
  flex: 0 0 var(--input-size-control-width);
}
